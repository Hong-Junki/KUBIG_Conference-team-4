"""
Category D — Stacking Ablation: SE include vs exclude (no-SE version)
======================================================================
Experiment : stacking_tree_only_12y_no_se
Ablation   : Same as stacking_tree_only_12y but WITHOUT macis_se_score feature.
             output/macis_12y/se_scores.parquet is never loaded.
             All other settings are identical to the SE baseline.

Policy     : full train — acled_missing_mask=1 rows included as training rows;
             acled_missing_mask excluded as a feature.
Base models: LightGBM (no SE), XGBoost (no SE)
...
Purpose    : Measure how much Macis 12y SE feature contributes to PR-AUC,
             P@top5%, Brier, and ECE. Compare with SE baseline outputs at end.

NOTE — Final base models use val for early stopping → val metrics mildly optimistic.
NOTE — Comparison report reads SE baseline from:
       outputs/reports/stacking_tree_only_12y_val_metrics.json

Usage (from project root):
    python modeling/run_stacking_d_no_se_ablation.py
"""
...
# ── Experiment identity ───────────────────────────────────────────────────────
EXPERIMENT   = "stacking_tree_only_12y_no_se"
TRAIN_POLICY = "full"     # same as SE baseline
OWNER        = "D_byeonghyeon"
RANDOM_SEED  = 42

# SE baseline experiment name (for comparison report)
SE_BASELINE_EXPERIMENT = "stacking_tree_only_12y"
...
# SE_PATH intentionally omitted — this ablation does not use SE scores
...
OUTPUT_FILES = {
    "oof_lgbm":         os.path.join(PRED_DIR,   f"oof_predictions__lgbm_no_se_12y__{OWNER}.csv"),
    "oof_xgb":          os.path.join(PRED_DIR,   f"oof_predictions__xgb_no_se_12y__{OWNER}.csv"),
    "val_lgbm":         os.path.join(PRED_DIR,   f"val_predictions__lgbm_no_se_12y__{OWNER}.csv"),
    "val_xgb":          os.path.join(PRED_DIR,   f"val_predictions__xgb_no_se_12y__{OWNER}.csv"),
    "test_lgbm":        os.path.join(PRED_DIR,   f"predictions__lgbm_no_se_12y__{OWNER}.csv"),
    "test_xgb":         os.path.join(PRED_DIR,   f"predictions__xgb_no_se_12y__{OWNER}.csv"),
    ...
    "model_lgbm":       os.path.join(MODEL_DIR,  "lgbm_no_se_12y_D_final.pkl"),
    "model_xgb":        os.path.join(MODEL_DIR,  "xgb_no_se_12y_D_final.pkl"),
    ...
    "cmp_csv":          os.path.join(REPORT_DIR, f"{SE_BASELINE_EXPERIMENT}_se_ablation_comparison.csv"),
    "cmp_md":           os.path.join(REPORT_DIR, f"{SE_BASELINE_EXPERIMENT}_se_ablation_comparison.md"),
}
...
# ─────────────────────────────────────────────────────────────────────────────
# Step 1 — Data loading (no SE merge)
# ─────────────────────────────────────────────────────────────────────────────

def load_all_data():
    print("\n[Step 1] 데이터 로딩 (SE 병합 없음 — no-SE ablation)")
    ...
    print("  → macis_se_score 피처 미포함 (SE ablation 목적)")
...
# ─────────────────────────────────────────────────────────────────────────────
# Step 7 — Comparison report (SE vs no-SE)
# ─────────────────────────────────────────────────────────────────────────────

def build_comparison(no_se_metrics):
    """Load SE baseline metrics and build side-by-side comparison."""
    se_json_path = os.path.join(REPORT_DIR, f"{SE_BASELINE_EXPERIMENT}_val_metrics.json")
    if not os.path.exists(se_json_path):
        print(f"  [경고] SE baseline 메트릭 파일 없음: {se_json_path}")
        return None, None

    with open(se_json_path) as f:
        se_data = json.load(f)
    se_metrics = se_data["models"]

    # Keys to compare
    model_pairs = [
        ("lgbm_se_12y",       "lgbm_no_se_12y",       "LightGBM"),
        ("xgb_se_12y",        "xgb_no_se_12y",        "XGBoost"),
    ...
    rows = []
    for se_key, no_se_key, label in model_pairs:
        se_m    = se_metrics.get(se_key, {})
        no_se_m = no_se_metrics.get(no_se_key, {})
        row = {"model": label}
        for mk in metrics_to_compare:
            row[f"se_{mk}"]    = round(se_m.get(mk, float("nan")), 4)
            row[f"no_se_{mk}"] = round(no_se_m.get(mk, float("nan")), 4)
            se_val    = se_m.get(mk, float("nan"))
            no_se_val = no_se_m.get(mk, float("nan"))
            if not (isinstance(se_val, float) and isinstance(no_se_val, float)):
                row[f"delta_{mk}"] = float("nan")
            else:
                row[f"delta_{mk}"] = round(se_val - no_se_val, 4)
        rows.append(row)
    ...
    def fmt(v):
        if isinstance(v, float) and not (v != v):  # not NaN
            return f"{v:+.4f}" if abs(v) >= 0.0001 else f"{v:.4f}"
        return str(v)
    ...
    lines = []
    lines.append("# SE vs no-SE 절제 비교 리포트")
    lines.append("")
    lines.append(f"> **기준**: `{SE_BASELINE_EXPERIMENT}` (SE 포함) vs `{EXPERIMENT}` (SE 없음)")
    lines.append(f"> **작성일**: 2026-05-23")
    lines.append(f"> **delta = SE − no-SE**: 양수이면 SE가 도움, 음수이면 SE가 역효과")
    lines.append("")
    lines.append("---")
    lines.append("")
    lines.append("## PR-AUC 비교")
    lines.append("")
    lines.append("| 모델 | SE 포함 | SE 없음 | delta (SE − no-SE) |")
    lines.append("|------|---------|---------|---------------------|")
    for row in rows:
        lines.append(
            f"| {row['model']} "
            f"| {row['se_pr_auc']:.4f} "
            f"| {row['no_se_pr_auc']:.4f} "
    ...
    lines.append("")
    lines.append("| 모델 | SE 포함 | SE 없음 | delta |")
    lines.append("|------|---------|---------|-------|")
    for row in rows:
        lines.append(
            f"| {row['model']} "
            f"| {row['se_p_at_top5pct']:.4f} "
            f"| {row['no_se_p_at_top5pct']:.4f} "
    ...
    lines.append("|------|---------|---------|---------------------|")
    for row in rows:
        d = row["delta_brier_score"]
        # For Brier: negative delta = SE has lower Brier = SE is better
        arrow = sign_arrow(-d) if d == d else "—"
        lines.append(
            f"| {row['model']} "
            f"| {row['se_brier_score']:.4f} "
            f"| {row['no_se_brier_score']:.4f} "
            f"| {sign_arrow(row['delta_brier_score'])} (SE brier 변화) |"
        )
    ...
    # Auto-generate conclusion based on stacking platt delta
    sp_delta = next(
        (r["delta_pr_auc"] for r in rows if r["model"] == "Stacking (Platt)"), float("nan")
    )
    lgbm_delta = next(
        (r["delta_pr_auc"] for r in rows if r["model"] == "LightGBM"), float("nan")
    )
    xgb_delta = next(
        (r["delta_pr_auc"] for r in rows if r["model"] == "XGBoost"), float("nan")
    )
    ...
    if sp_delta == sp_delta:  # not NaN
        if sp_delta > 0.005:
            lines.append("> **결론**: SE 피처가 스태킹 PR-AUC를 유의미하게 개선함. **SE 유지 권장.**")
        elif sp_delta < -0.005:
            lines.append("> **결론**: SE 피처 포함 시 오히려 PR-AUC가 하락. **SE 제거 고려 필요.**")
        else:
            lines.append("> **결론**: SE 피처의 PR-AUC 기여가 미미함 (±0.005 이내). "
                         "ECE·Brier 등 보정 지표도 함께 판단 필요.")
...
def save_all(oof_lgbm, oof_xgb, preds, val_stack_df,
    ...
    oof_summary = {
        ...
        "note":            "2014-2017 rows excluded from OOF. No SE feature (ablation).",
    ...
    val_metrics_out = {
        ...
        "note_ablation":             "SE feature excluded. Compare with stacking_tree_only_12y for SE contribution.",
        "models":                    all_metrics,
    }
    ...
    md_rows = "\n".join([
        mrow("LightGBM (no SE)",    all_metrics["lgbm_no_se_12y"]),
        mrow("XGBoost (no SE)",     all_metrics["xgb_no_se_12y"]),
...
"""
    ...
    if cmp_df is not None:
        ...
        with open(OUTPUT_FILES["cmp_md"], "w", encoding="utf-8") as f:
            f.write(cmp_md)
        print("  SE vs no-SE 비교 리포트 저장: 2개")
    else:
        print("  [경고] SE baseline 메트릭 없음 — 비교 리포트 생략")
...
def main():
    print("=" * 70)
    print(f"  [카테고리 D] SE Ablation: {EXPERIMENT}")
    print(f"  정책: {TRAIN_POLICY} | Base: LightGBM + XGBoost | SE: 없음")
    print("=" * 70)

    check_existing_outputs()
    ensure_output_dirs()

    # ── 1. Data loading (no SE merge) ─────────────────────────────────────────
    ...
    print(f"  피처 수: {len(feature_cols)}  (SE 컬럼: {se_cols if se_cols else '없음 ✓'})")
    ...
    val_y = preds["val_y"]
    all_metrics = {
        "lgbm_no_se_12y":    compute_metrics(val_y, preds["val_l"]),
        "xgb_no_se_12y":     compute_metrics(val_y, preds["val_x"]),
    ...
    labels = {
        "lgbm_no_se_12y":    "LightGBM (no SE)     ",
        "xgb_no_se_12y":     "XGBoost (no SE)      ",
    ...
    # ── 10. Comparison with SE baseline ───────────────────────────────────────
    print("\n[Step 10] SE baseline 대비 비교 리포트 생성")
    ...
    print()
    print("=" * 70)
    print("  실행 완료 — SE ablation")
    print(f"  최고 val PR-AUC (no-SE) : {best_pr:.4f}")
