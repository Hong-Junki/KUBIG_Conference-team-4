"""
Category D — Train Start Date Ablation: 2016-start (8y) vs 2014-start (12y)
=============================================================================
Experiment : stacking_tree_only_8y_with_mask_feature
Ablation   : Restrict train data to 2016-01-01 onward (8 years instead of 12).
             Tests whether removing early noisy / low-ACLED-coverage years
             (2014-2015, where mask=1 rate is 40-49%) improves performance.
             All other settings identical to stacking_tree_only_12y_with_mask_feature.

Policy     : full train — acled_missing_mask=1 rows included as training rows;
             acled_missing_mask INCLUDED as a feature (same as 12y best).
Base models: LightGBM + 8y-windowed train + SE + mask,
             XGBoost  + 8y-windowed train + SE + mask
...
OOF design : expanding-window, 6 folds, SAME target years as 12y baseline.
  F1: train 2016-2017 → predict 2018   (12y had 84,738 rows; 8y has 42,398)
  F2: train 2016-2018 → predict 2019   (12y had 105,908;    8y has  63,568)
  F3: train 2016-2019 → predict 2020   (12y had 127,078;    8y has  84,738)
  F4: train 2016-2020 → predict 2021   (12y had 148,306;    8y has 105,966)
  F5: train 2016-2021 → predict 2022   (12y had 169,476;    8y has 127,136)
  F6: train 2016-2022 → predict 2023   (12y had 190,646;    8y has 148,306)
  Meta-learner trains on 2018-2023 OOF rows (identical to 12y design).

Purpose    : Measure whether restricting training to 2016+ improves PR-AUC,
             P@top5%, Brier, ECE compared to 12y_with_mask_feature.
             delta = 8y − 12y. Positive = shorter window helps.

NOTE — Final base models use val for early stopping → val metrics mildly optimistic.
NOTE — Comparison reads baseline from:
       outputs/reports/stacking_tree_only_12y_with_mask_feature_val_metrics.json

Usage (from project root):
    python modeling/run_stacking_d_train2016_ablation.py
"""
...
TRAIN_START_YEAR = 2016         # ablation variable: restrict train to 2016+
OWNER            = "D_byeonghyeon"
RANDOM_SEED      = 42

# Baseline experiment for comparison (current best D-category model)
BASELINE_EXPERIMENT = "stacking_tree_only_12y_with_mask_feature"
...
# acled_missing_mask intentionally NOT excluded — kept as feature (same as 12y best)
ALWAYS_EXCLUDE = [DATE_COL]

# ── OOF fold design: same pred target years as 12y baseline ──────────────────
# Train window starts from TRAIN_START_YEAR (2016) instead of 2014.
OOF_FOLDS = [
...
# ── Hyperparameters (identical to 12y_with_mask_feature baseline) ─────────────
LGB_PARAMS = {
    "objective":         "binary",
...
REFERENCE_PR_AUC = 0.2714   # 12y_with_mask_feature Stacking Platt (current best)

# ── Output file registry ──────────────────────────────────────────────────────
_8WMF = "8y_wmf"   # shorthand for filename suffix (base model files)
OUTPUT_FILES = {
    "oof_lgbm":         os.path.join(PRED_DIR,   f"oof_predictions__lgbm_se_{_8WMF}__{OWNER}.csv"),
    "oof_xgb":          os.path.join(PRED_DIR,   f"oof_predictions__xgb_se_{_8WMF}__{OWNER}.csv"),
    "val_lgbm":         os.path.join(PRED_DIR,   f"val_predictions__lgbm_se_{_8WMF}__{OWNER}.csv"),
    "val_xgb":          os.path.join(PRED_DIR,   f"val_predictions__xgb_se_{_8WMF}__{OWNER}.csv"),
    "test_lgbm":        os.path.join(PRED_DIR,   f"predictions__lgbm_se_{_8WMF}__{OWNER}.csv"),
    "test_xgb":         os.path.join(PRED_DIR,   f"predictions__xgb_se_{_8WMF}__{OWNER}.csv"),
    ...
    "model_lgbm":       os.path.join(MODEL_DIR,  f"lgbm_se_{_8WMF}_D_final.pkl"),
    "model_xgb":        os.path.join(MODEL_DIR,  f"xgb_se_{_8WMF}_D_final.pkl"),
    ...
    "cmp_csv":          os.path.join(REPORT_DIR, "stacking_tree_only_12y_train_start_ablation_comparison.csv"),
    "cmp_md":           os.path.join(REPORT_DIR, "stacking_tree_only_12y_train_start_ablation_comparison.md"),
}
...
# ─────────────────────────────────────────────────────────────────────────────
# Step 1 — Data loading and train-window restriction
# ─────────────────────────────────────────────────────────────────────────────

def load_all_data():
    print(f"\n[Step 1] 데이터 로딩 — train을 {TRAIN_START_YEAR}년 이후로 제한 (8y ablation)")
    ...
    print(f"  train (원본): {len(train_raw):,}행 | "
          f"{train_raw[DATE_COL].min().date()} ~ {train_raw[DATE_COL].max().date()}")

    # ── Train start restriction ───────────────────────────────────────────────
    train = train_raw[train_raw[DATE_COL].dt.year >= TRAIN_START_YEAR].copy()
    n_removed = len(train_raw) - len(train)
    print(f"  → {TRAIN_START_YEAR}+ 필터 적용: {len(train):,}행 유지 "
          f"(제거된 2014-2015 행: {n_removed:,}행, "
          f"{n_removed / len(train_raw) * 100:.1f}%)")
    print(f"  → train 날짜 범위: {train[DATE_COL].min().date()} ~ {train[DATE_COL].max().date()}")
    print(f"  → train 양성률: {train[TARGET_COL].mean():.4f}")
    ...
    print(f"  → acled_missing_mask=1 행 (2016+): {mask1_train:,}행 "
          f"({mask1_train / len(train) * 100:.1f}%) — 피처로 포함, 학습에 포함")
...
def run_oof_folds(train_df, feature_cols, country_enc):
    print(f"\n[Step 2] OOF 생성 (expanding-window, 6 folds, train start={TRAIN_START_YEAR})")
    ...
    for fold in OOF_FOLDS:
        ...
        # train_df already starts from TRAIN_START_YEAR — filter upper bound only
        ...
        print(f"\n  {fname}: 학습 {n_tr:,}행 ({TRAIN_START_YEAR}~{train_end}) "
        ...
        fold_summaries.append({
            ...
            "train_start": TRAIN_START_YEAR, "train_end": train_end,
...
def train_final_lgbm(train_df, val_df, feature_cols):
    print(f"\n[Step 3a] Final LightGBM 학습 (train {TRAIN_START_YEAR}+, val early stopping)")
...
def train_final_xgb(train_df, val_df, feature_cols, country_enc):
    print(f"\n[Step 3b] Final XGBoost 학습 (train {TRAIN_START_YEAR}+, val early stopping)")
...
# ─────────────────────────────────────────────────────────────────────────────
# Step 7 — Comparison report: 8y vs 12y train-start ablation
# ─────────────────────────────────────────────────────────────────────────────

def build_comparison(new_metrics):
    """Load 12y_with_mask_feature metrics and build 8y vs 12y comparison.
    delta = 8y − 12y. Positive = shorter window (2016-start) helps.
    """
    base_json_path = os.path.join(
        REPORT_DIR, f"{BASELINE_EXPERIMENT}_val_metrics.json"
    )
    ...
    # (12y_key, 8y_key, display_label)
    model_pairs = [
        ("lgbm_se_12y_with_mask_feature", "lgbm_se_8y_with_mask_feature", "LightGBM"),
        ("xgb_se_12y_with_mask_feature",  "xgb_se_8y_with_mask_feature",  "XGBoost"),
    ...
    rows = []
    for base_key, new_key, label in model_pairs:
        base_m = base_metrics.get(base_key, {})
        new_m  = new_metrics.get(new_key, {})
        row = {"model": label}
        for mk in metrics_to_compare:
            row[f"y12_{mk}"] = round(base_m.get(mk, float("nan")), 4)
            row[f"y8_{mk}"]  = round(new_m.get(mk, float("nan")), 4)
            b_val = base_m.get(mk, float("nan"))
            n_val = new_m.get(mk, float("nan"))
            if not (isinstance(b_val, (int, float)) and isinstance(n_val, (int, float))):
                row[f"delta_{mk}"] = float("nan")
            else:
                row[f"delta_{mk}"] = round(n_val - b_val, 4)  # 8y − 12y
        rows.append(row)

    cmp_df = pd.DataFrame(rows)

    def sign_arrow(d, lower_is_better=False):
        if d != d: return "—"
        if lower_is_better:
            if d < -0.0005: return f"{d:.4f} ↓ (개선)"
            if d >  0.0005: return f"+{d:.4f} ↑ (악화)"
            return f"{d:.4f} ≈"
        else:
    ...
    today = "2026-05-23"
    lines = []
    lines.append("# Train 시작일 절제 비교 리포트 (12y vs 8y)")
    lines.append("")
    lines.append(f"> **기준 (12y)**: `{BASELINE_EXPERIMENT}` (train 2014-2023)")
    lines.append(f"> **비교 (8y)**: `{EXPERIMENT}` (train 2016-2023)")
    lines.append(f"> **작성일**: {today}")
    lines.append(f"> **delta = 8y − 12y**: 양수이면 2016-start가 도움, 음수이면 역효과")
    lines.append("")
    lines.append("---")
    lines.append("")
    lines.append("## PR-AUC 비교")
    lines.append("")
    lines.append("| 모델 | 12y (2014-start) | 8y (2016-start) | delta (8y − 12y) |")
    lines.append("|------|------------------|-----------------|------------------|")
    for row in rows:
        lines.append(
            f"| {row['model']} "
    ...
    lines.append("")
    lines.append("| 모델 | 12y | 8y | delta |")
    lines.append("|------|-----|-----|-------|")
    for row in rows:
        lines.append(
            f"| {row['model']} "
            f"| {row['y12_p_at_top5pct']:.4f} "
            f"| {row['y8_p_at_top5pct']:.4f} "
    ...
    lines.append("")
    lines.append("| 모델 | 12y | 8y | delta (8y − 12y) |")
    lines.append("|------|-----|-----|------------------|")
    for row in rows:
        lines.append(
            f"| {row['model']} "
            f"| {row['y12_brier_score']:.4f} "
            f"| {row['y8_brier_score']:.4f} "
            f"| {sign_arrow(row['delta_brier_score'], lower_is_better=True)} |"
        )
    ...
    for row in rows:
        lines.append(
            ...
            f"| {row['y12_ece']:.4f} "
            f"| {row['y8_ece']:.4f} "
            f"| {sign_arrow(row['delta_ece'], lower_is_better=True)} |"
        )
    ...
    sp_y8      = next((r["y8_pr_auc"]    for r in rows if r["model"] == "Stacking (Platt)"), float("nan"))
    sp_y12     = next((r["y12_pr_auc"]   for r in rows if r["model"] == "Stacking (Platt)"), float("nan"))

    lines.append(f"- **LightGBM PR-AUC delta (8y − 12y)**: {sign_arrow(lgbm_delta)}")
    lines.append(f"- **XGBoost PR-AUC delta (8y − 12y)**: {sign_arrow(xgb_delta)}")
    lines.append(f"- **Stacking Platt PR-AUC (12y)**: {sp_y12:.4f}")
    lines.append(f"- **Stacking Platt PR-AUC (8y)**: {sp_y8:.4f}")
    ...
    if sp_delta == sp_delta:  # not NaN
        if sp_delta > 0.005:
            verdict = (
                f"> **결론**: 2016-start(8y)가 2014-start(12y) 대비 스태킹 PR-AUC를 "
                f"유의미하게 개선함 (+{sp_delta:.4f}). "
                f"**2016-start 채택 권장.** 현재 최선: {EXPERIMENT} + Platt (PR-AUC {sp_y8:.4f})."
            )
        elif sp_delta < -0.005:
            verdict = (
                f"> **결론**: 2016-start(8y)가 오히려 PR-AUC 하락 ({sp_delta:.4f}). "
                f"**2014-start(12y) 유지 권장.** 2014-2015 데이터가 유용한 신호를 포함하고 있음."
            )
        else:
            verdict = (
                f"> **결론**: 8y vs 12y 차이가 미미함 (delta={sp_delta:.4f}, ±0.005 이내). "
                f"Brier·ECE 보조 지표 및 fold별 추이도 함께 판단 필요."
            )
    ...
    lines.append("")
    lines.append("1. `stacking_tree_only_6y_with_mask_feature` — 2018-start 절제 실험 (5-fold OOF 필요)")
    lines.append("2. C담당 LSTM OOF 파일 수령 시 BASE_MODELS에 추가 → 최종 앙상블 재실험")
    lines.append("")
...
def save_all(oof_lgbm, oof_xgb, preds, val_stack_df,
    ...
    oof_summary = {
        ...
        "note": (
            "Train restricted to 2016+. OOF target years 2018-2023 same as 12y baseline. "
            "acled_missing_mask included as feature. Meta-learner trains on 2018-2023 OOF."
        ),
    ...
    val_metrics_out = {
        ...
        "se_feature_included":     True,
        "best_meta_C":             best_c,
        "reference_12y_pr_auc":    REFERENCE_PR_AUC,
        "note_early_stopping":     "Final base models use val for early stopping; val metrics mildly optimistic.",
        "note_ablation":           "Train restricted to 2016+. Compare with 12y_with_mask_feature for train-window contribution.",
        "models":                  all_metrics,
    }
    ...
    # Val metrics MD
    stacking_pr = all_metrics.get("stacking_platt", {}).get(
        "pr_auc", all_metrics.get("stacking_raw", {}).get("pr_auc", 0.0))
    improved = "✓ 12y 대비 개선됨" if stacking_pr > REFERENCE_PR_AUC else "✗ 12y 대비 미개선"
    ...
    md_rows = "\n".join([
        mrow("LightGBM + SE + mask (8y/2016+)", all_metrics["lgbm_se_8y_with_mask_feature"]),
        mrow("XGBoost  + SE + mask (8y/2016+)", all_metrics["xgb_se_8y_with_mask_feature"]),
...
**Train 윈도우**: 8y ({TRAIN_START_YEAR}-2023, 12y 대비 2014-2015 제외)
...
**비교 기준**: 12y_with_mask_feature Stacking Platt PR-AUC = {REFERENCE_PR_AUC}
...
- 12y_with_mask_feature Stacking Platt PR-AUC: **{REFERENCE_PR_AUC}**
- 8y_with_mask_feature  Stacking Platt PR-AUC: **{stacking_pr:.4f}**
- 결과: **{improved}**
"""
    ...
    if cmp_df is not None:
        ...
        with open(OUTPUT_FILES["cmp_md"], "w", encoding="utf-8") as f:
            f.write(cmp_md)
        print("  Train-start 비교 리포트 저장: 2개")
    else:
...
def main():
    print("=" * 70)
    print(f"  [카테고리 D] Train-start Ablation: {EXPERIMENT}")
    print(f"  Train 윈도우: {TRAIN_START_YEAR}~2023 (8y) | SE: 포함 | mask 피처: 포함")
    print(f"  비교 기준: {BASELINE_EXPERIMENT} (12y, PR-AUC {REFERENCE_PR_AUC})")
    print("=" * 70)

    check_existing_outputs()
    ensure_output_dirs()

    # ── 1. Data loading (with 2016+ restriction) ──────────────────────────────
    ...
    assert "acled_missing_mask" in feature_cols, \
        "acled_missing_mask must be a feature in this ablation"
    ...
    val_y = preds["val_y"]
    all_metrics = {
        "lgbm_se_8y_with_mask_feature": compute_metrics(val_y, preds["val_l"]),
        "xgb_se_8y_with_mask_feature":  compute_metrics(val_y, preds["val_x"]),
    ...
    labels = {
        "lgbm_se_8y_with_mask_feature": "LightGBM + SE + mask (8y)",
        "xgb_se_8y_with_mask_feature":  "XGBoost  + SE + mask (8y)",
        "stacking_raw":                 "Stacking (raw)           ",
        "stacking_platt":               "Stacking (Platt)         ",
        "stacking_isotonic":            "Stacking (Isotonic)      ",
    }
    print(f"\n  {'모델':<30} {'PR-AUC':>8} {'P@5%':>8} {'Brier':>8} {'ECE':>8}")
    print(f"  {'-'*62}")
    for k, lb in labels.items():
        m    = all_metrics[k]
        flag = " ← best" if m["pr_auc"] == max(x["pr_auc"] for x in all_metrics.values()) else ""
        print(f"  {lb:<30} {m['pr_auc']:>8.4f} {m['p_at_top5pct']:>8.4f} "
              f"{m['brier_score']:>8.4f} {m['ece']:>8.4f}{flag}")

    # ── 10. Comparison with 12y baseline ──────────────────────────────────────
    print("\n[Step 10] 12y baseline 대비 비교 리포트 생성")
    ...
    sp_pr = all_metrics["stacking_platt"]["pr_auc"]
    delta = sp_pr - REFERENCE_PR_AUC
    arrow = f"+{delta:.4f} ↑" if delta > 0 else f"{delta:.4f} ↓"
    verdict = "개선됨 → 2016-start 채택 고려" if delta > 0.005 else \
              "미개선 → 12y 유지 권장" if delta < -0.005 else "차이 미미 → 추가 지표 판단 필요"

    print()
    print("=" * 70)
    print("  실행 완료 — Train-start 2016 ablation")
    print(f"  Stacking Platt val PR-AUC (8y) : {sp_pr:.4f}")
    print(f"  Stacking Platt val PR-AUC (12y): {REFERENCE_PR_AUC}")
    print(f"  delta (8y − 12y)               : {arrow}")
    print(f"  판정                            : {verdict}")
    print("=" * 70)
