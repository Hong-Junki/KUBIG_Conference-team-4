"""
Experiment D: ACLED-free + GDELT Title + Theme/Person Features
==============================================================
C feature set (57개) 위에 GDELT v2themes / v2persons 기반 피처 22개를 추가한다.
...
Feature set (79개):
  ...
  D 추가 피처 (22개):
    1d (11): gdelt_theme_nonnull_count_1d, gdelt_person_nonnull_count_1d,
             gdelt_theme_count_1d, gdelt_person_count_1d,
             gdelt_theme_{conflict,protest,military,refugee,sanction,government}_count_1d,
             gdelt_person_density_1d
    7d (11): 위 11개의 7d rolling sum/mean
...
D theme/person 피처 소스:
  members/byeonghyeon/input/processed/gdelt_titles/gdelt_theme_person_features.parquet
  (build_gdelt_theme_person_features.py로 생성, gitignore로 추적 안 됨)
...
출력:
  members/byeonghyeon/outputs/reports/acled_free_with_theme_person_results.md
  (예측 CSV 저장 없음)

실행:
  cd <KUBIG_Conference-team-4 루트>
  python members/byeonghyeon/modeling/run_stacking_acled_free_with_theme_person.py

  Smoke test (~5분):
  SMOKE_TEST=1 python members/byeonghyeon/modeling/run_stacking_acled_free_with_theme_person.py
"""
...
# ── 실험 식별자 ────────────────────────────────────────────────────────────────
EXPERIMENT  = "stacking_acled_free_with_theme_person" + ("_smoke" if SMOKE_TEST else "")
RANDOM_SEED = 42
...
GDELT_THEME_PERSON_PATH = os.path.join(_SUBTREE_ROOT, "input", "processed", "gdelt_titles",
                                       "gdelt_theme_person_features.parquet")

# ── 출력 경로 ─────────────────────────────────────────────────────────────────
REPORT_DIR = os.path.join(_SUBTREE_ROOT, "outputs", "reports")
REPORT_MD  = os.path.join(REPORT_DIR, "acled_free_with_theme_person_results.md")
...
BASELINE_C2_PR_AUC = 0.0643  # 미채택, 참고용
ADOPT_THRESHOLD    = BASELINE_C_PR_AUC + 0.003  # ≥ 0.0683

EXPECTED_N_FEATURES = 79  # B:35 + C:21 + mask:1 + D:22
...
def validate_setup():
    ...
    print(f"  GDELT_THEME_PERSON_PATH: {GDELT_THEME_PERSON_PATH}")
    print(f"  REPORT_MD              : {REPORT_MD}")
    print(f"  B baseline PR-AUC      : {BASELINE_B_PR_AUC}")
    print(f"  C baseline PR-AUC      : {BASELINE_C_PR_AUC}")
    print(f"  C2 PR-AUC (미채택)     : {BASELINE_C2_PR_AUC}")
    ...
    if not os.path.exists(GDELT_TITLE_PATH):
        ...
        print("  먼저 실행하세요: python members/byeonghyeon/modeling/build_gdelt_title_features.py")
        sys.exit(1)

    if not os.path.exists(GDELT_THEME_PERSON_PATH):
        print(f"\n[오류] GDELT theme/person 피처 parquet 없음: {GDELT_THEME_PERSON_PATH}")
        print("  먼저 실행하세요: python members/byeonghyeon/modeling/build_gdelt_theme_person_features.py")
        sys.exit(1)
...
# ─────────────────────────────────────────────────────────────────────────────
# Step 1 — 데이터 로드 + C title + D theme/person 피처 merge
# ─────────────────────────────────────────────────────────────────────────────

def load_all_data():
    print("\n[Step 1] 데이터 로딩 + GDELT title merge + theme/person merge")
    ...
    # C: GDELT title 피처 로드
    gdelt_title_df = pd.read_parquet(GDELT_TITLE_PATH)
    print(f"\n  GDELT title (C)      : {len(gdelt_title_df):,}행 | "
          f"{gdelt_title_df['date'].min().date()} ~ {gdelt_title_df['date'].max().date()} | "
          f"{gdelt_title_df['country'].nunique()}개국")

    # D: theme/person 피처 로드
    gdelt_tp_df = pd.read_parquet(GDELT_THEME_PERSON_PATH)
    print(f"  GDELT theme/person (D): {len(gdelt_tp_df):,}행 | "
          f"{gdelt_tp_df['date'].min().date()} ~ {gdelt_tp_df['date'].max().date()} | "
          f"{gdelt_tp_df['country'].nunique()}개국")

    # 날짜 타입 정규화
    for df in [train, val, test, gdelt_title_df, gdelt_tp_df]:
        if df["date"].dt.tz is None:
            df["date"] = df["date"].dt.tz_localize("UTC")

    title_merge = gdelt_title_df[["date", "country"] + GDELT_TITLE_FEATURE_COLS]
    tp_merge    = gdelt_tp_df[["date", "country"] + GDELT_THEME_PERSON_FEATURE_COLS]

    def merge_all(split_df, split_name):
        n_before = len(split_df)

        # C title merge
        merged = split_df.merge(title_merge, on=["date", "country"], how="left")
        assert len(merged) == n_before, f"row count changed after C merge [{split_name}]"
        n_null_c = merged[GDELT_TITLE_FEATURE_COLS].isnull().sum().sum()
        ...
        # D theme/person merge
        merged = merged.merge(tp_merge, on=["date", "country"], how="left")
        assert len(merged) == n_before, f"row count changed after D merge [{split_name}]"
        n_null_d = merged[GDELT_THEME_PERSON_FEATURE_COLS].isnull().sum().sum()
        merged[GDELT_THEME_PERSON_FEATURE_COLS] = merged[GDELT_THEME_PERSON_FEATURE_COLS].fillna(0)

        n_covered = int((merged[GDELT_TITLE_COVERAGE_COL] == 1).sum())
        print(f"    [{split_name}] C null {n_null_c:,}→0 | D null {n_null_d:,}→0 | "
...
def get_feature_cols(df):
    """ACLED/SE/label/future 제외 + C title + coverage_mask + D theme/person 반환."""
    all_c = set(GDELT_TITLE_FEATURE_COLS)
    all_d = set(GDELT_THEME_PERSON_FEATURE_COLS)
...
def validate_feature_cols(feature_cols):
    ...
    d_in  = [c for c in GDELT_THEME_PERSON_FEATURE_COLS if c in feature_cols]
    ...
    gdelt_ev = [c for c in feature_cols
                ...
                and c not in set(GDELT_THEME_PERSON_FEATURE_COLS)
                and c != GDELT_TITLE_COVERAGE_COL]
    ...
    print(f"    D: theme/person          : {len(d_in)}개  (기대 22)")
    ...
    if len(d_in) != 22:
        print(f"  [경고] D theme/person feature 수 불일치: {len(d_in)} ≠ 22")
...
def save_report(all_metrics, best_c, fold_summaries, feature_cols):
    ...
    lines = [
        f"# 실험 D: ACLED-free + GDELT Title + Theme/Person Features 결과{smoke_tag}",
        f"",
        f"**실험명**: `{EXPERIMENT}`  ",
        f"**실행일**: {today}  ",
        f"**B baseline val PR-AUC**: {BASELINE_B_PR_AUC} (참고)  ",
        f"**C baseline val PR-AUC**: {BASELINE_C_PR_AUC} (비교 기준)  ",
        f"**C2 val PR-AUC (미채택)**: {BASELINE_C2_PR_AUC} (참고)  ",
    ...
    c_n       = sum(1 for c in GDELT_TITLE_FEATURE_COLS if c in feature_cols)
    mask_n    = 1 if GDELT_TITLE_COVERAGE_COL in feature_cols else 0
    d_n       = sum(1 for c in GDELT_THEME_PERSON_FEATURE_COLS if c in feature_cols)
    ...
    lines += [
        ...
        f"| GDELT title 1d/7d (C)       | {c_n} |",
        f"| coverage_mask (C)           | {mask_n} |",
        f"| GDELT theme/person (D 신규) | {d_n} |",
        f"| **D 총계**                  | **{len(feature_cols)}** |",
        "",
        "---",
        "",
        "## val 지표 비교 (B / C / C2 / D)",
    ...
    c2_ref = {"lgbm": 0.0642, "xgb": 0.0566, "stacking_raw": 0.0643,
              "stacking_platt": BASELINE_C2_PR_AUC, "stacking_isotonic": 0.0636}
    ...
    for key, label in model_order:
        if key not in all_metrics:
            continue
        mv    = all_metrics[key]
        d_pr  = mv.get("pr_auc", float("nan"))
        c_pr  = c_ref.get(key, float("nan"))
        c2_pr = c2_ref.get(key, float("nan"))
        b_pr  = b_ref.get(key, float("nan"))
        dc    = delta_str(d_pr, c_pr)  if (d_pr == d_pr and c_pr == c_pr) else "—"
        dc2   = delta_str(d_pr, c2_pr) if (d_pr == d_pr and c2_pr == c2_pr) else "—"
        db    = delta_str(d_pr, b_pr)  if (d_pr == d_pr and b_pr == b_pr) else "—"
        lines.append(
            f"| {label} "
            f"| {fmt(d_pr)} | {c_pr:.4f} | {dc} "
            f"| {c2_pr:.4f} | {dc2} "
            f"| {b_pr:.4f} | {db} "
...
def main():
    ...
    print(f"  최종 feature_cols: {len(feature_cols)}개 "
          f"(B 35 + C title 21 + coverage_mask 1 + D 22 = {EXPECTED_N_FEATURES})")
    ...
    test_stack_raw = meta_model.predict_proba(  # noqa: F841  (test는 저장 안 함)
    ...
    print(f"  C baseline PR-AUC     : {BASELINE_C_PR_AUC}")
    print(f"  C2 PR-AUC (미채택)    : {BASELINE_C2_PR_AUC}")
    print(f"  D Stacking Platt      : {platt_pr:.4f}")
    print(f"  delta (D - C)         : {delta_c:+.4f}")
    print(f"  delta (D - B)         : {delta_b:+.4f}")
