"""
Experiment C: ACLED-free + GDELT Title Features
================================================
B baseline (GDELT events + 경제 + country, 35개 피처) 위에
GDELT title/tone/count/domain/language aggregate feature 22개를 추가한다.

목적:
  B baseline (val PR-AUC=0.0564) 대비 +0.003 이상 개선되는지 확인한다.
...
Feature set (57개):
  ...
  추가 피처 (22개):
    ...
    Coverage (1):        gdelt_title_coverage_mask  (1 = 2015-02-17 이전)
...
OOF design: B/Reference와 동일한 expanding-window 6-fold
...
# ── 실험 식별자 ────────────────────────────────────────────────────────────────
EXPERIMENT  = "stacking_acled_free_with_titles" + ("_smoke" if SMOKE_TEST else "")
RANDOM_SEED = 42
...
REPORT_MD  = os.path.join(REPORT_DIR, "acled_free_with_titles_results.md")
...
# GDELT title 피처 21개 (coverage_mask 제외)
...
# B baseline PR-AUC (채택 기준 비교용)
BASELINE_B_PR_AUC  = 0.0564
ADOPT_THRESHOLD    = BASELINE_B_PR_AUC + 0.003  # ≥ 0.0594
...
# ─────────────────────────────────────────────────────────────────────────────
# Step 1 — 데이터 로드 + GDELT title 피처 merge
# ─────────────────────────────────────────────────────────────────────────────

def load_all_data():
    print("\n[Step 1] 데이터 로딩 + GDELT title 피처 merge")
    ...
    print(f"\n  GDELT title : {len(gdelt_df):,}행 | "
          f"{gdelt_df['date'].min().date()} ~ {gdelt_df['date'].max().date()} | "
          f"{gdelt_df['country'].nunique()}개국")

    # 날짜 타입 정규화 (train은 UTC-aware, gdelt_df도 UTC-aware로 저장됨)
    for df in [train, val, test, gdelt_df]:
        if df["date"].dt.tz is None:
            df["date"] = df["date"].dt.tz_localize("UTC")

    # left-join: GDELT title 피처 병합
    merge_cols = GDELT_TITLE_FEATURE_COLS  # 21개
    gdelt_merge = gdelt_df[["date", "country"] + merge_cols]

    def merge_gdelt(split_df, split_name):
        n_before = len(split_df)
        merged = split_df.merge(gdelt_merge, on=["date", "country"], how="left")
        assert len(merged) == n_before, f"row count changed after merge [{split_name}]"

        # 결측 → 0 (보도 없는 날 또는 coverage gap)
        n_null = merged[merge_cols].isnull().sum().sum()
        merged[merge_cols] = merged[merge_cols].fillna(0)
        ...
        print(f"    [{split_name}] null {n_null:,}개 → 0 채움 | "
              f"coverage_mask=1: {n_covered:,}행 ({n_covered/len(merged)*100:.1f}%)")
        return merged

    print()
    train = merge_gdelt(train, "train")
    val   = merge_gdelt(val,   "val")
    test  = merge_gdelt(test,  "test")
...
def get_feature_cols(df):
    """ACLED/SE/label/future 제외 + GDELT title 추가 feature_cols 반환."""
    exclude = set(LABEL_META_COLS) | set(ALWAYS_EXCLUDE)
    base_cols = [c for c in df.columns if c not in exclude
                 and c not in GDELT_TITLE_FEATURE_COLS
                 and c != GDELT_TITLE_COVERAGE_COL]
    return base_cols + GDELT_TITLE_FEATURE_COLS + [GDELT_TITLE_COVERAGE_COL]
...
def validate_feature_cols(feature_cols):
    ...
    # 피처 수 검증
    b_in_c = [c for c in FEATURE_COLS_B if c in feature_cols]
    gdelt_title_in_c = [c for c in GDELT_TITLE_FEATURE_COLS if c in feature_cols]
    has_mask = GDELT_TITLE_COVERAGE_COL in feature_cols

    print(f"  feature_cols ({len(feature_cols)}개):")
    gdelt_ev = [c for c in feature_cols if c.startswith("gdelt_") and c not in GDELT_TITLE_FEATURE_COLS and c != GDELT_TITLE_COVERAGE_COL]
    econ     = [c for c in feature_cols if c.startswith("econ_")]
    print(f"    B feature (GDELT events) : {len(gdelt_ev)}개")
    print(f"    B feature (economic)     : {len(econ)}개")
    print(f"    B feature (country)      : {'country' in feature_cols}")
    print(f"    B feature 합계           : {len(b_in_c)}개  (기대 35)")
    print(f"    GDELT title feature      : {len(gdelt_title_in_c)}개  (기대 21)")
    print(f"    coverage_mask            : {has_mask}  (기대 True)")
    print(f"  ACLED/SE/label 잔존 없음 ✅")

    if len(b_in_c) != 35:
        print(f"  [경고] B feature 수 불일치: {len(b_in_c)} ≠ 35")
    if len(gdelt_title_in_c) != 21:
        print(f"  [경고] GDELT title feature 수 불일치: {len(gdelt_title_in_c)} ≠ 21")
...
def save_report(all_metrics, best_c, fold_summaries, feature_cols):
    ...
    lines = [
        f"# 실험 C: ACLED-free + GDELT Title Features 결과",
        f"",
        f"**실험명**: `{EXPERIMENT}`  ",
        f"**실행일**: {today}  ",
        ...
        f"B baseline({BASELINE_B_PR_AUC}) 대비 {delta_str(platt_pr, BASELINE_B_PR_AUC)}",
        "",
        "---",
        "",
        "## 피처 구성",
    ...
    gdelt_ev_n = sum(1 for c in feature_cols if c.startswith("gdelt_") and c not in GDELT_TITLE_FEATURE_COLS and c != GDELT_TITLE_COVERAGE_COL)
    ...
    lines += [
        ...
        f"| GDELT title (신규)  | {title_n} |",
        f"| coverage_mask (신규)| {mask_n} |",
        f"| **C 합계**          | **{len(feature_cols)}** |",
        "",
        "---",
        "",
        "## val 지표 비교 (B vs C)",
    ...
    for key, label in model_order:
        if key not in all_metrics:
            continue
        mv = all_metrics[key]
        c_pr = mv.get("pr_auc", float("nan"))
        b_pr = b_ref.get(key, float("nan"))
        d    = delta_str(c_pr, b_pr) if (c_pr == c_pr and b_pr == b_pr) else "—"
        lines.append(
            f"| {label} "
            f"| {fmt(c_pr)} "
            f"| {b_pr:.4f} "
    ...
    lines += [
        ...
        f"2. 채택 기준: B + 0.003 = {ADOPT_THRESHOLD:.4f}. 이를 초과하면 실험 D (themes/persons)로 진행.",
...
def main():
    ...
    # Step 1: 데이터 로드 + GDELT title merge
    ...
    print(f"  최종 feature_cols: {len(feature_cols)}개 "
          f"(B 35 + GDELT title 21 + coverage_mask 1 = 57)")
    ...
    print(f"  C Stacking Platt      : {platt_pr:.4f}")
    delta = platt_pr - BASELINE_B_PR_AUC
    print(f"  delta (C - B)         : {delta:+.4f}")
    print(f"  채택 여부             : {'✅ 채택 (≥ '+str(ADOPT_THRESHOLD)+')' if adopted else '❌ 미채택 (< '+str(ADOPT_THRESHOLD)+')'}")
